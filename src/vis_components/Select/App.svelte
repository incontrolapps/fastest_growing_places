<script>
 ...
 fetch('./data/' + e + '.json')
      ...
      .then(e=>document.getElementById('canvas').scrollIntoView({
  behavior: 'smooth'
}))
  }
...
<div>
  <div class="ons-field">
   <!-- <label class="ons-label" for="select-wide">
      Select a local authority area from the map or menu
    </label>-->
    <select
      bind:value={selected}
      id="select-la"
      ...
      <option value="null">
        {hovered ? lookup[hovered] : 'No local authority selected yet'}
      </option>
